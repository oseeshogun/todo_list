<template>
    <div>
        ...
        <form @submit="submitTask" @reset="() => onCancelEdition()" method="POST" action=""
            ...
            <div class="flex items-center my-0">
                ...
                <input v-model="text" type="text" @blur="() => onCancelEdition()" autofocus
                    class="w-full mr-3 p-1 border-sky-500 outline-sky-500" v-if="editing" />
                <p v-else @click.stop="editing = true"
                    :class="{ 'line-through': task.finished, 'text-gray-400': task.finished }"
